...
// The WhatsApp group join is NO LONGER mandatory — it lives as an optional
// "join" card on the dashboard (see WhatsAppGroupCard).
...
const ProfileCompletionGate = () => {
  ...
  useEffect(() => {
    setOpen(!!isLoggedIn && needsCompletion(user));
  }, [isLoggedIn, user]);

  // `user` can flip to null (logout / session expiry) before the decide-effect
  // clears `open` — guard so we never read user.role off null.
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await dispatch(getUser()); // needsCompletion → false closes the gate
